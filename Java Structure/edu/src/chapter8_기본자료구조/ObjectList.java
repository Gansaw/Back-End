...
class SimpleObject {
	...
	private static class NoOrderComparator implements Comparator<SimpleObject> {
		// compare을 사용할 수 있는 이유 -> interface이기 때문에 method역할을 가지고 있음
...
class LinkedList2 {
	...
	public int Delete(SimpleObject element, Comparator<SimpleObject> cc) {
		...
		while (p != null) {
			// data에 있는 값이랑 입력한 element랑 같은 경우 => 삭제 가능
		...
		// p가 null값인 경우 => 삭제 불가능 => -1로 return
		return -1;
	}

	public void Show() { // 전체 리스트를 순서대로 출력한다.3
		Node2 p = first;
		while (p != null) {
