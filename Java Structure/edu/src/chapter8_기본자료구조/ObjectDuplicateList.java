...
// --- 노드를 검색 ---//
	public boolean search(SimpleObject2 obj, Comparator<? super SimpleObject2> c) {
	    Node4 p = first.rlink;
	    while (p != first) {
	        if (p.data != null && c.compare(p.data, obj) == 0) {
	            // 데이터 found
	            return true;
	        }
	        p = p.rlink;
...
// --- list에 삭제할 데이터가 있으면 해당 노드를 삭제 ---//
	...
	public DoubledLinkedList2 merge(DoubledLinkedList2 lst2) {
		DoubledLinkedList2 mergeList = new DoubledLinkedList2();
		Node4 p = first.rlink;
		Node4 q = lst2.first.rlink;

		while (p != first) {
			mergeList.add(new SimpleObject2(p.data.no, p.data.name), SimpleObject2.NO_ORDER);
			p = p.rlink;
		}
		while (q != lst2.first) {
			mergeList.add(new SimpleObject2(q.data.no, q.data.name), SimpleObject2.NO_ORDER);
			q = q.rlink;
		}
...
public class ObjectDuplicateList {
	enum Menu {
